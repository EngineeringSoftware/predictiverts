...
public class BashUtils {
    ...
    public static RunResult run(String cmd, Integer expectedReturnCode) {
        try {
            ...
            String stdout = new BufferedReader(new InputStreamReader(proc.getInputStream())).lines().collect(Collectors.joining("\n"));
            String stderr = new BufferedReader(new InputStreamReader(proc.getErrorStream())).lines().collect(Collectors.joining("\n"));
            ...
            if (expectedReturnCode != null && exitCode != expectedReturnCode) {
                ...
                throw new RuntimeException("Expected " + expectedReturnCode + " but returned " + exitCode + " while executing bash command '" + cmd + "'.\n" +
                        "stdout: " + stdout + "\n" +
                        "stderr: " + stderr);
            }
