...
public abstract class Config {
    ...
    public static <T extends Config> T load(Path configPath, Class<T> clz) {
        try {
     ...
     */
    public static <T extends Config> T load(Reader reader, Class<T> clz) {
        try {
            ...
            for (Field f : clz.getFields()) {
                if (f.getAnnotation(Option.class) != null) {
                    ...
                    if (!SUPPORTED_CLASSES_2_READERS.containsKey(fType)) {
                        throw new RuntimeException("Unsupported option type " + fType + ", for field " + f.getName() + " in class " + clz);
                    }
            ...
            if (configJson.isJsonObject()) {
                for (Map.Entry<String, JsonElement> entry : configJson.getAsJsonObject().entrySet()) {
                    Pair<Class<?>, Field> p = options.get(entry.getKey());
                    if (p != null) {
                        p.getRight().set(config, SUPPORTED_CLASSES_2_READERS.get(p.getLeft()).apply(entry.getValue()));
                    } else {
